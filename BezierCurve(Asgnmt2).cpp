...
void mouseClickingButton(GLFWwindow* window, int button, int action, int mods) {
    ...
    glfwGetCursorPos(window, &horPosition, &verPosition);           //Get the address of the horizontal and vertical position itself, not the value
...
int main() {
    ...
    GLuint shaderProgram, shaderProgram2, shaderProgram3; // Required to load shaders, 
    shaderProgram = LoadShaders("vertex.glsl", "fragment.glsl", "tess_control.glsl", "tess_eval.glsl"); // For normal 4 points, shaders
    shaderProgram2 = LoadShaders("vertex.glsl", "fragment.glsl", "tess_control2.glsl", "tess_eval.glsl");
    shaderProgram3 = LoadShaders("vertex.glsl", "fragment.glsl", "tesscontrol3.glsl", "tess_eval.glsl");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (fixPoints.size() >= 2 && fixPoints.size() <= 4) {
            GLuint selectedShader = 0;

            switch (fixPoints.size()) {
            case 2: selectedShader = shaderProgram2; break;
            case 3: selectedShader = shaderProgram3; break;
            case 4: selectedShader = shaderProgram; break;
